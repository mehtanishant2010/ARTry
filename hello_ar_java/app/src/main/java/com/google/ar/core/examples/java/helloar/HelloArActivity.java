...
public class HelloArActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  private void handleTap(Frame frame, Camera camera) {
    ...
    Float fling = tapHelper.pollF();
    if (fling != null) {
      //Toast.makeText(this.getBaseContext(), "Scale " + scaleFactor, Toast.LENGTH_LONG).show();
      if (fling < 0 && scaleFactor > 0.3) {
        scaleFactor -= 0.2;
      }
      else if (fling > 0 && scaleFactor < 5) {
        scaleFactor += 0.2;
      }
      return;
//      float x1 = tap.getX(0);
...
//      float x2 = tap.getX(1);
//      float y2 = tap.getY(1);
//      double dist1 = Math.sqrt((x1-x2)*(x1-x1) + (y1-y2)*(y1-y2));
//      float x3 = tap.getHistoricalX(0);
//      float y3 = tap.getHistoricalY(0);
//      float x4 = tap.getHistoricalX(1);
//      float y4 = tap.getHistoricalY(1);
//      double dist2 = Math.sqrt((x3-x4)*(x3-x4) + (y3-y4)*(y3-y4));
//      if (dist2 > dist1 && dist2 - dist1 > 2) {
//        if (scaleFactor > 0.2) {
//          scaleFactor -= 0.1;
//          return;
//        }
